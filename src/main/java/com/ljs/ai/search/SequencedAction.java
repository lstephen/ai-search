 ...
 * @author lstephen
 */
public class SequencedAction<S extends State> extends Action<S> {
    ...
    public static <S extends State> SequencedAction<S> create(Action<S>... as) {
        return new SequencedAction<S>(Arrays.asList(as));
    }

    private static <S extends State> SequencedAction<S> create(Iterable<Action<S>> actions) {
        return create(Iterables.get(actions, 0), Iterables.get(actions, 1));
    }

    public static <S extends State> ImmutableSet<SequencedAction<S>> allPairs(Iterable<? extends Action<S>> actions) {
    ...
    public static <S extends State> ImmutableSet<SequencedAction<S>> merged(
